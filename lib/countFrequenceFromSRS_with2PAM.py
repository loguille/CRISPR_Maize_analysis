            ...
            if keys-len_primer_forward < 1 or keys > len(sequence_target)-len_primer_reverse :
                ...
                for i in range(len(sort_dico_mut[keys])):
                    if sort_dico_mut[keys][i][0] == 'insertion' :
                        ...
                        '''
                        if keys-len_primer_forward >= start_pam1 and keys-len_primer_forward <= end_pam1 or keys-len_primer_forward >= start_pam2 and keys-len_primer_forward <= end_pam2 :
                        ...
                        if keys-len_primer_forward >= start_pam1 and keys-len_primer_forward <= end_pam1 or keys-len_primer_forward+len(sort_dico_mut[keys][i][1]) >= start_pam1 and keys-len_primer_forward+len(sort_dico_mut[keys][i][1]) <= end_pam1 or keys-len_primer_forward >= start_pam2 and keys-len_primer_forward <= end_pam2 or keys-len_primer_forward+len(sort_dico_mut[keys][i][1]) >= start_pam2 and keys-len_primer_forward+len(sort_dico_mut[keys][i][1]) <= end_pam2 :
                            long_file.write(str(keys-len_primer_forward)+'\t'+str(keys-len_primer_forward+len(sort_dico_mut[keys][i][1]))+'\t'+sort_dico_mut[keys][i][0]+'\t'+sort_dico_mut[keys][i][1]+'\t/\t'+str(sort_dico_mut[keys][i][2])+'\t'+str(-len(sort_dico_mut[keys][i][1]))+'\t'+str(int(sort_dico_mut[keys][i][2]*nbsequence))+'\t'+str(float(sort_dico_mut[keys][i][3])/float(correctedNbseq))+'\t'+str(sort_dico_mut[keys][i][3])+'\tYes\n')
